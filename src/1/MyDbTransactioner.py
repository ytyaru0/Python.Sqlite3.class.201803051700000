...
#class MyDbTransactioner(Transactioner):
#    def __init__(self, db_url, *dataset_args, **dataset_kwargs):
#        super().__init__(db_url, *dataset_args, **dataset_kwargs)

class MyDbTransactioner:
    T = Transactioner('sqlite:///mydb.db')
    
    @T.transact(MyDbTransactioner)
    def CreateTable(cls, db, table_name, name_types):
    ...
    @T.transact(MyDbTransactioner)
    #@transact
    def Insert(cls, db, table_name, **kv):
